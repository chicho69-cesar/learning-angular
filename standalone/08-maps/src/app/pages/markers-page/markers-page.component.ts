import { JsonPipe } from '@angular/common';
import { AfterViewInit, Component, ElementRef, signal, viewChild } from '@angular/core';
import mapboxgl, { LngLatLike } from 'mapbox-gl';
import { v4 as uuid } from 'uuid';

import { environment } from '../../../environments/environment';
import { type Marker as MyMarker } from '../../types/maps.interface';

mapboxgl.accessToken = environment.mapboxKey;

@Component({
  selector: 'app-markers-page',
  imports: [JsonPipe],
  templateUrl: './markers-page.component.html',
  styleUrl: './markers-page.component.css'
})
export class MarkersPageComponent implements AfterViewInit {
  public mapElement = viewChild<ElementRef<HTMLDivElement>>('map');
  public map = signal<mapboxgl.Map | null>(null);
  public markers = signal<MyMarker[]>([]);

  public async ngAfterViewInit() {
    if (!this.mapElement()?.nativeElement) return;
    await new Promise((resolve) => setTimeout(resolve, 100));

    const element = this.mapElement()!.nativeElement;

    const map = new mapboxgl.Map({
      container: element,
      style: 'mapbox://styles/mapbox/streets-v12',
      center: [-122.40985, 37.793085],
      zoom: 14,
    });

    // const marker = new mapboxgl.Marker({
    //   draggable: false,
    //   color: '#000',
    // })
    //   .setLngLat([-122.40985, 37.793085])
    //   .addTo(map);

    // marker.on('dragend', (event) => {
    //   console.log(event);
    // });

    this.mapListeners(map);
  }

  public mapListeners(map: mapboxgl.Map) {
    map.on('click', (event) => this.mapClick(event));
    this.map.set(map);
  }

  public mapClick(event: mapboxgl.MapMouseEvent) {
    if (!this.map()) return;

    const map = this.map()!;
    const coords = event.lngLat;
    const color = '#xxxxxx'.replace(/x/g, (y) =>
      ((Math.random() * 16) | 0).toString(16)
    );

    const mapboxMarker = new mapboxgl.Marker({
      color: color,
    })
      .setLngLat(coords)
      .addTo(map);

    const newMarker: MyMarker = {
      id: uuid(),
      mapboxMarker: mapboxMarker,
    };

    this.markers.update((current) => [...current, newMarker]);
  }

  public flyToMarker(lngLat: LngLatLike) {
    if (!this.map()) return;

    this.map()!.flyTo({
      center: lngLat,
    });
  }

  public deleteMarker(marker: MyMarker) {
    if (!this.map()) return;
    // const map = this.map()!;

    marker.mapboxMarker.remove();
    this.markers.update((current) => current.filter((m) => m.id !== marker.id));
  }
}
