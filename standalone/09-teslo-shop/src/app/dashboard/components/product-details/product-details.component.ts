...
@Component({
  selector: 'dashboard-product-details',
...
export class ProductDetailsComponent implements OnInit {
  ...
  public setFormValue(partialProduct: Partial<Product>) {
    this.productForm.reset(this.product() as any);
    this.productForm.patchValue({ tags: partialProduct.tags?.join(', ') });
  }
  ...
  public onFilesChanged(event: Event) {
    ...
    const imagesUrls = Array.from(this.imageFileList ?? []).map((file) => {
  ...
  public async onSubmit() {
    ...
    if (partialProduct.id === 'new') {
      const product = await firstValueFrom(
        this.productsService.createProduct(partialProduct, this.imageFileList)
      );

      this.router.navigate(['/admin/products', product.id]);
    } else {
      await firstValueFrom(
        this.productsService.updateProduct(this.product().id, partialProduct, this.imageFileList)
      );
    }
